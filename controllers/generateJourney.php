...
$journeySchema = array();
$start = $_SESSION['parameters']['start'];
$end = $_SESSION['parameters']['end'];
...
echo "Schema du voyage : ";
echo "<pre>";
print_r($journeySchema);
echo "</pre>";
...
for($i = 0; $i < count($journeySchema); $i++){
    if($journeySchema[$i]['type'] != 'D'){
        if($journeySchema[$i]['P/S'] == 'P'){
            $journeySchema[$i]['candidates'] = getPlacesFromPrimaryType($journeySchema[$i]['id'], $_SESSION["candidates"][0]["geometry"]["location"], 10000);
            ...
            while(count($journeySchema[$i]['candidates']) == 0){
                ...
                if($journeySchema[$i]['P/S'] == 'P'){
                    ...
                    $journeySchema[$i]['candidates'] = getPlacesFromSecondaryType($journeySchema[$i]['id'], $_SESSION["candidates"][0]["geometry"]["location"], 10000);
                }
                $security++;
                if($security > 5){
                    break;
