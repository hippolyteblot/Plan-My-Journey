...
function register($firstname, $lastname, $email, $password, $confirmPassword) {
    ...
    if (isset($check['messageAlert'])) {
        ...
        $database = Connexion::getInstance()->getBdd();
        $query = $database->prepare('INSERT INTO Compte (prenom, nom, email, mot_de_passe VALUES (?, ?, ?, ?)');
        $query->execute(array($firstname, $lastname, $email, password_hash($password, PASSWORD_DEFAULT)));
